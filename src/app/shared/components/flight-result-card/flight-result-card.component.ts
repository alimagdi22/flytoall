...
import { FlightCheckoutService, FlightResultService, IAirItinerary } from 'rp-travel-ui';
...
export class FlightResultCardComponent {
  ...
  private loadBrandedFaresData(): void {
    ...
    if (data) {
      ...
      this.flightResultService.getBrandedFares(
        this.sharedService.searchParams['searchId'],
        this.sharedService.selectedCardData[0].segmentNums,
        this.sharedService.selectedCardData[0].pKey,
        this.sharedService.selectedCardData[0].pcc,
      );
    }
  }
  ...
  getBrandedBaggageCount(index: number, passengerType: string): string {
    ...
    for (const passengerBaggage of passengerBaggageList) {
      ...
      const detail = passengerBaggage.baggageAllowanceDetails.find((detail: any) => detail.flightRoute === flightRoute);
