...
import { FlightCheckoutService, IAirItinerary, ISearchCriteria } from 'rp-travel-ui';
...
export class FlightInfoComponent implements OnInit {
  ...
  goToFlightDetails(airItinerary: IAirItinerary) {
    ...
    this.flightSharedService.selectedFlight = [airItinerary];
    
    const flightsCount = airItinerary.allJourney?.flights?.length || 0;
    if (flightsCount === 1) {
      this.flightSharedService.selectedFlightType = 'oneway';
    } else if (flightsCount === 2) {
      this.flightSharedService.selectedFlightType = 'roundtrip';
    } else if (flightsCount > 2) {
      this.flightSharedService.selectedFlightType = 'multicity';
    }
