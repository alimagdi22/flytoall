import { Component, EventEmitter, inject, OnInit, Output, AfterViewInit, OnDestroy } from '@angular/core';
...
export class StepTwoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private setupScrollObserver(): void {
    ...
    this.observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          switch (id) {
            case 'contact-details-section':
              this.sharedService.checkoutSteps = 0;
              break;
            case 'travellers-details-section':
              this.sharedService.checkoutSteps = 1;
              break;
            case 'payment-method-section':
              this.sharedService.checkoutSteps = 2;
              break;
          }
        }
      });
    ...
    const travellersSection = document.getElementById('travellers-details-section');
    const paymentSection = document.getElementById('payment-method-section');

    if (contactSection) this.observer.observe(contactSection);
    if (travellersSection) this.observer.observe(travellersSection);
    if (paymentSection) this.observer.observe(paymentSection);
  }
